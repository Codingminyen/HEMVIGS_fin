import sys, time, os, tqdm, argparse, glob, subprocess, warnings, cv2, pickle, numpy, pdb, math
...
parser.add_argument('--videoPath',             type=str, default='./demo/demo_video.mp4',   help='Demo video path')
parser.add_argument('--saveDir',               type=str, default='./results',  help='Path for face track inputs, tmps and outputs')
parser.add_argument('--mode',                  type=str, default='cuda',  help='cuda or cpu')
...
def face_track_main(args):
    ...
    args.savePath = os.path.join(args.saveDir, os.path.basename(args.videoPath).split('.')[0])
    ...
    # Face tracking
    allTracks = []
    allTracks.extend(track_shot(args, faces))
